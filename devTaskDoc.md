...
<img src="https://i.loli.net/2020/11/25/bvVgLux13KqzapA.png" alt="nothing" style="zoom: 25%;" />



<img src="https://i.loli.net/2020/11/25/7vdVk6LEBifnhDZ.png" alt="login" style="zoom:25%;" />





2：底部导航栏小纸条图标种类和样式需要重新选择和优化（解决）

3:主页内容详情页图片无法点开查看（解决）

4:动态页话题详情页所有图片都没有添加（解决）
...
1：类别导航栏没有固定，导致上拉时会出现移动，期望效果是不移动（解决）

2：首页顶部栏左右两个图标没有对齐，大小也不一致，期望效果是要样式位置和动态页一致（解决）

3：小纸条页每一个聊天用户列表的时间位置和聊天数量标记位置没有对齐，时间位置过于靠边，聊天数量标记样式有待优化（解决）

4：个人主页的没有内容替代图片位置过于处在下方（解决）

5：个人设置页修改密码页和修改邮箱页输入框以及文字提示过于靠左，期望效果是可以和微信设计一样无论是输入框还是提示文字和左边边框都应当有有一些距离

6：发布页输入内容过于靠边，期望效果是输入内容可以距离顶部和左边都应当有一些距离(解决)

==让所有终端的页面开发都在自己的掌控之中，需要一步一步，慢慢接触==

##### 微信小程序端

重要，取消微信原生顶部栏

![image-20201125224656661](https://i.loli.net/2020/11/25/iVwpkjGNe2ROglD.png)



在取消微信的原生导航栏之后，需要额外在页面上添加样式来维持小程序页面的稳定，但是添加的该样式将会在并不需要的APP端和H5端展现而影响这两个终端的样式，**需要找到一个方法来动态判断是否为微信小程序端，如果是再来添加稳定小程序页面的样式**

建议解决方案一：

是尝试直接在`pages.json`文件中修改`mp-wweixin`来调整顶部栏样式，而`mp-weixin`的配置直接来源于微信官方小程序API，实践证明该解决方案无效，微信官方API没有提供对应固定位置的API

建议解决方案二：

[uni-app跨端兼容](https://uniapp.dcloud.io/platform?id=%E8%B7%A8%E7%AB%AF%E5%85%BC%E5%AE%B9)

![image-20201126094352127](https://i.loli.net/2020/11/26/1Gi7ItXEovKwZhC.png)
...
![image-20201126084056417](https://i.loli.net/2020/11/26/IELn5lND9V274rM.png)



直接在封装的公共组件类别导航栏上设置`position:fixed`

但是该做法的副作用是遇到只有两三个类别时会遇到样式位置问题，出现在微信小程序和H5，预计也会在APP端出现问题，所以应当在要在对应页面来确定是否开启fixed,主要问题出现在
...
==学习建议：使用uni-app开发尽量使用官方组件库，实践证明通常情况下使用官方组件库可以很好的适配多端==



下一阶段任务

每一个功能页面的数据库设计  --->  后台接口的开发 
