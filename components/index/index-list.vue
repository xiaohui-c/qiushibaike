<template>
  <view>
    <view
      class="index-list animate__animated animate__fadeInUp"
    ...
    >
      ...
      <view class="index-list3" @tap="opendetail(item.title)">
        ...
        <template v-if="item.type == 'video'">
          ...
          <view class="index-list-playinfo"
            >{{ item.playnum }}次播放 {{ item.long }}</view
          >
        </template>
      </view>
      <!-- 底部点赞评论 -->
      <view class="index-list4 u-f u-f-ac u-f-jc">
        ...
        <view class="u-f u-f-ac u-f-jc">
          <view
            class="u-f u-f-ac u-f-jc"
            :class="[item.infonum.index == 1 ? 'active' : '']"
            @tap="control('ding', index)"
            ><view class="icon iconfont icon-icon_xiaolian-mian"></view
            >{{ item.infonum.dingnum }}</view
          >
          <view
            class="u-f u-f-ac u-f-jc"
            :class="[item.infonum.index == 2 ? 'active' : '']"
            @tap="control('cai', index)"
            ><view class="icon iconfont icon-kulian"></view
            >{{ item.infonum.cai }}</view
          >
        </view>
        <!-- 底部右边评论转发 -->
        <view class="u-f u-f-ac u-f-jc">
